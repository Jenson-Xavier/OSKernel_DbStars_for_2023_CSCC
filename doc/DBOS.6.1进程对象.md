正如前面总体设计中所说，进程的设计是采取模块和管理器分离的设计，这一点似乎有点参考了“前后端分离”的设计和架构模式。整个内核中进程管理部分中每一个进程都由一个进程控制块PCB来维护，这在本代码中是一个结构体，具体代码如下。
```cpp
struct proc_struct
{
...
```
具体每个成员的含义一旁都有清楚的注释，这里就不重复介绍了。其中在进程控制块中，有一个成员`stat`是表示进程当前的状态的，这个状态的具体值我们使用了枚举结构来实现，可见如下代码。我们对进程主要设计了六个状态，分配态、就绪态、执行态、睡眠态/挂起态、终止态、僵死态。这些状态在进程的创建和执行的生命周期中会不断进行切换和变化，从而保证内核能够对每个进程根据它们的状态进行正确的执行逻辑。
```cpp
enum proc_status
{
...
```
	对于每个进程控制块我们的`ProcessManager`类中提供了大量的来操作进程结构体指针的函数接口，这些接口的声明和含义如下可见，注释中都很清楚，我们也没有篇幅对于每个函数接口都进行详细的说明，在后面的部分会对某些重要核心的接口进行说明，事实上我们的注释中都有详实的解释。	
```cpp
